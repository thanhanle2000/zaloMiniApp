      ...
      <Swiper slidesPerView={2.25} spaceBetween={12} className="px-4">
        {patterns.map((category) => (
          <SwiperSlide key={category.lable}>
            <div className=" relative pb-[10px] flex flex-col items-center bg-white border-b border-r border-slate-300  rounded-lg  p-1 ">
              <div
                className="relative w-full aspect-[5/6] rounded-lg bg-cover bg-center bg-skeleton"
                style={{ backgroundImage: `url(${category.image})` }}
              />
              <span className="mt-1 text-sm font-bold text-slate-700 py-2">{category.lable}</span>
              <div onClick={() => gotoCategory(category.id)} className=" bg-[#0074BC] px-2 py-[2px] rounded-lg">
